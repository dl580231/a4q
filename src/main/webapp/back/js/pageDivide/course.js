...
function initCourse() {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.state == 0) {
				var tempHtml = '<tr></th>' + '<th>课程ID</th>' + '<th>版主信息</th>'
						+ '<th>课程名称</th>' + '<th>创建时间</th>' + '<th>最后修改时间</th>'
						+ '<th>操作</th></tr>';
				$.map(data.data, function(value, index) {
					tempHtml += '<tr>' + '<td>' + value.courseId + '</td>'
							+ '<td>' + moderatorJudgeNull(value.moderator)
							+ '</td>' + '<td>' + value.courseName + '</td>'
