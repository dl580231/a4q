		...
		function search(currentPageLocal){
			var queryUserUrl = '/a4q/personInfoAdmin/getPersonInfoList?currentPage='+currentPageLocal;
					...
					if (data.state == 0) {
						...
						$.map(pageDivide.entityList, function(value, index) {
							tempHtml+='<tr><td><input type="checkbox" name="IDCheck" value="'+value.userId+'" class="acb" /></td><td>'+
							 ...
							 '</td><td>'+value.email+'</td><td>'+value.userType+'</td><td>'+value.lable+'</td><td>'+format(value.createTime)+'</td><td>'
							 +format(value.lastEditTime)+'</td><td><a href="user_edit.html?userId='+value.userId+'&fyID=14458579642011" class="edit">编辑</a>/'+
